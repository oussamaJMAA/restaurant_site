...
<img src="img/recipie/<?php echo $row['image']; ?>"  />
<div class="rc-info">
...
<?php
    ...
    $pr=$row['id_recette'];
	$db = config::getConnexion();
        $selecta=$db->prepare( "SELECT AVG(rate) AS average FROM rate_recette where id_recette = :uid " ); 
		$selecta->execute(array(':uid' =>$pr));
		while($row2=$selecta->fetch(PDO::FETCH_ASSOC)){
      
    ?>


<?php echo "rate : ". number_format((float)$row2['average'], 2, '.', '');  ?> /5
...
<form method="post" action="recipie_details.php">



<input type="hidden"  name="idr" value="<?php echo $row['id_recette']  ?>" >
<button type="submit" name="add" class="btn btn-default">View Details</button>
</form> 
<hr>
</div>
...
<p><i class="fa fa-envelope-o"></i><a href="https://demo.web3canvas.com/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="87f4f2f7f7e8f5f3c7f5e2f4f3e6f2f5e6e9f3a9e4e8ea">[email&#160;protected]</a></p>
</div>
</div>
</div>
</div>
...
<!-- Mirrored from demo.web3canvas.com/themeforest/tomato/recipe.html by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 04 Apr 2021 00:15:47 GMT -->
</html>
